...
class InviteHeaderViewModel : ViewModel() {
    val dataList = mutableListOf<IInviteModel>()
    ...
    fun getFirstPage(pageSize: Int = 10) {
        dataList.clear()
        currentPage = 0
        dataList.add(JoinedInvitationModel(1, LocalDateTime.now().plusMinutes(3), "北校区篮球场打篮球"))
        dataList.add(InviteHeaderModel(1, LocalDateTime.now(), "测试标题", "测试用户",
            "测试内容", null, "https://img-blog.csdn.net/column?imageView2/1/w/224/h/224/interlace/1", 233, 123))
        dataList.add(InviteHeaderModel(2, LocalDateTime.now().minusMinutes(24), "网传北理工理教发生自杀事件，是真的吗？发生什么了？", "连城",
